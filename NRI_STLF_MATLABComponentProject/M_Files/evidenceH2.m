function [net, gamma, logev] = evidenceH2(net, x, t, num)
...
for k = 1 : num
  ...
  if size(net.alpha) == [1 1]
    ...
    for m = 1 : ngroups
      ...
      %net.alpha(m) = real(abs(gams(m))/(2*eprior(m)));
  ...
  % Re-estimate beta.
%   if isfield(net, 'beta')
%       %net.beta = abs(0.5*(net.nout*ndata - gamma)/edata);
%       net.beta = 0.5*(net.nout*ndata - gamma)/edata;
%       %logev = logev + 0.5*ndata*log(net.beta) - 0.5*ndata*log(2*pi);
%       logev = logev + 0.5*ndata*log(net.beta) + 0.5*log(2/gamma) + ...
%           0.5*log(2/(net.nout*ndata-gamma));
%       local_beta = net.beta;
%   end
